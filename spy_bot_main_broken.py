...
async def stats_command(interaction: discord.Interaction):
    """Show spy bot statistics (Owner only - detailed stats)."""
    
    try:
        # Check if user is the owner
        if interaction.user.id == OWNER_ID:
                # OWNER VIEW - Detailed statistics
                data = load_spy_data()
                shield_data = load_shield_data()
                notifications_data = load_notifications()
                tz_data = load_user_timezones()
                ...
                alliances = {}
                users_who_added = {}
                user_report_counts = {}  # Track how many reports each user has added
                guilds_with_reports = {}
                ...
                users_who_added[user_id] = username
            
                # Count reports per user
                if user_id not in user_report_counts:
                    user_report_counts[user_id] = {'username': username, 'count': 0}
                user_report_counts[user_id]['count'] += 1
            
                guild_id = report['added_by'].get('guild_id')
                guild_name = report['added_by'].get('guild_name', 'Unknown')
                if guild_id:
                    if guild_id not in guilds_with_reports:
                        guilds_with_reports[guild_id] = {'name': guild_name, 'count': 0}
                    guilds_with_reports[guild_id]['count'] += 1
        
            # Also count shields per user
            user_shield_counts = {}
            for shield in shield_data["shields"]:
                user_id = shield['user_id']
                username = shield['username']
                if user_id not in user_shield_counts:
                    user_shield_counts[user_id] = {'username': username, 'count': 0}
                user_shield_counts[user_id]['count'] += 1
        
            # Combine report and shield counts for total usage
            user_total_usage = {}
            for user_id, report_data in user_report_counts.items():
                user_total_usage[user_id] = {
                    'username': report_data['username'],
                    'reports': report_data['count'],
                    'shields': user_shield_counts.get(user_id, {}).get('count', 0)
                }
            # Add users who only have shields (no reports)
            for user_id, shield_data in user_shield_counts.items():
                if user_id not in user_total_usage:
                    user_total_usage[user_id] = {
                        'username': shield_data['username'],
                        'reports': 0,
                        'shields': shield_data['count']
                    }
        
            # Calculate total usage (reports + shields)
            for user_id in user_total_usage:
                user_total_usage[user_id]['total'] = (
                    user_total_usage[user_id]['reports'] + 
                    user_total_usage[user_id]['shields']
                )
        
            # Sort by count
            top_alliances = sorted(alliances.items(), key=lambda x: x[1], reverse=True)[:5]
            top_guilds = sorted(guilds_with_reports.items(), key=lambda x: x[1]['count'], reverse=True)[:5]
            top_users = sorted(user_total_usage.items(), key=lambda x: x[1]['total'], reverse=True)[:10]
            ...
            # Notification stats
            users_with_notifications = sum(1 for u in notifications_data["users"].values() if u.get("enabled", False))
        
            embed = discord.Embed(
                title="📊 Spy Bot Statistics (Owner View)",
                description="Detailed bot statistics and usage",
            ...
            embed.add_field(
                ...
                value=f"**Total Reports:** {len(reports)}\n"
                      f"**Active Users:** {len(users_who_added)}\n"
            ...
            embed.add_field(
                name="🛡️ Shield Stats",
                value=f"**Active Shields:** {active_shields}\n"
                      f"**Total Shield Records:** {len(shield_data['shields'])}\n"
                      f"**Users w/ Notifications:** {users_with_notifications}",
                inline=True
            )
        
            if top_guilds:
                guild_list = "\n".join([f"**{info['name']}**: {info['count']} report(s)" for gid, info in top_guilds])
                embed.add_field(
                    name="🏆 Top Servers by Reports",
                    value=guild_list,
                    inline=False
                )
            ...
            if top_users:
                users_list = "\n".join([
                    f"**{user_data['username']}**: {user_data['total']} action(s) "
                    f"({user_data['reports']} reports + {user_data['shields']} shields)"
                    for uid, user_data in top_users
                ])
                embed.add_field(
            ...
            # Server list
            if guild_count > 0:
                server_list = "\n".join([f"• {guild.name} ({guild.member_count} members)" for guild in bot.guilds[:5]])
                if guild_count > 5:
                    server_list += f"\n... and {guild_count - 5} more"
                embed.add_field(
            ...
            embed.set_footer(text=f"Owner: {interaction.user} • Bot by KnyCat")
        
        await interaction.response.send_message(embed=embed, ephemeral=True)
    
    else:
        # PUBLIC VIEW - Basic statistics
        data = load_spy_data()
        shield_data = load_shield_data()
        reports = data["reports"]
